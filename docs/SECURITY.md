...
| Weak passwords                     | Zod regex enforces 8+ chars with upper / lower / number / symbol.                      |
| Brute-force login                  | Per-(IP,email) Redis rate-limit on register/login + global `express-rate-limit`.       |
| Brute-force OTP                    | Max 5 attempts per OTP; 5th wrong attempt locks the account for 15 minutes.             |
| Token theft via XSS                | `accessToken` and `refreshToken` cookies are `httpOnly`; no auth state in localStorage. |
| CSRF                               | Double-submit cookie + custom header (`x-csrf-token`); enforced on every mutation route. |
| Session hijacking                  | Single-active-session: a fresh login deletes any other session for that user.           |
| Replay of revoked tokens           | Refresh tokens are stored in Redis and compared on `/refresh`; revoked = unusable.      |
...
| Slow / oversized request abuse     | `express.json({ limit: "100kb" })` + `compression()`.                                   |
...
| Server starting in a broken state  | `validateEnv()` + fail-fast on Mongo / Redis connect errors.                             |
...
- **OTP comparison uses `!==`, not a constant-time comparison.** OTP is a short numeric secret with a 5-attempt budget, so timing-attack feasibility is low — but `crypto.timingSafeEqual` would be better.
- **No device tracking.** Single-active-session means *the most recent device wins*. We don't display "previous session was on Chrome / Mac / IP x.x.x.x to the user".
...
5. Restart the API. All existing sessions (signed by the old `JWT_SECRET` / `REFRESH_SECRET`) will fail verification, forcing every user to log in again — this is intentional after a key rotation.
