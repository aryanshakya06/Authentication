...
> A production-style **MERN authentication service**. Email verification, JWT access and refresh rotation, Redis-backed single-active-session enforcement, CSRF protection, OTP-based 2FA, account lockout, password reset, and a polished React 19 frontend with hand-rolled CSS (no UI framework).
...
- **JWT access and refresh tokens with Redis-backed sessions.** 15-minute access, 7-day refresh, both as `httpOnly` cookies. A single active session per user is enforced. Logging in elsewhere invalidates the previous session immediately.
...
- **CSRF protection.** Double-submit cookie plus custom header. Tokens auto-refresh through the axios interceptor.
- **Password reset flow.** Time-boxed Redis token, signs the user out of all devices on success.
- **Security middleware.** Helmet, compression, environment-aware cookies, global and per-auth-route rate limiting, request-id correlation, central error handler.
- **Senior-grade structure.** Clean `src/` layout, separation of routes, controllers, services, middleware, validators, templates and utils. ApiError class. asyncHandler that funnels into one error middleware.
- **Multi-theme UI (6 palettes).** `data-theme` attribute on `<html>` driven by CSS custom properties. Three light themes (Authly, Rose, Mint) and three dark (Purple Night, Midnight Blue, Obsidian). Persisted in `localStorage`. FOUC-free on reload via a blocking inline script.
...
- **Hand-rolled component CSS.** Reusable `Button`, `Input`, `PasswordInput`, `Spinner`, `FormCard`. `Navbar` (with `Themes` button) and `Footer` shell. `ErrorBoundary`, `ProtectedRoute`, `PublicOnlyRoute`. Real OTP input with autofocus, numeric-only mask and resend cooldown timer. Inter font. Accessible labels and focus rings.
...
| Frontend | React 19, Vite 7, react-router v7, axios, react-toastify, plain CSS      |
...
│     ├─ pages/                      # Landing, Home, Login, Register, VerifyOTP,
│     │                              # Verify, Forgot/Reset, Dashboard, NotFound
...
The server boots on `http://localhost:5000`. Hit `http://localhost:5000/api/v1/health` and you should see `{ "status": "ok", "mongo": "ok", "redis": "ok" }`.
...
Open `http://localhost:5173`. That is the public **landing page**. Click **Themes** in the top-right to flip between the 6 palettes.
...
5. **Enter OTP**. On success you receive `accessToken`, `refreshToken`, and `csrfToken` cookies and are dropped on `/home`.
...
Authly ships **6 themes** (3 dark, 3 light) selectable from the **Themes** button in the navbar. The default is `purple-night`.

| Theme            | Mode  | Accent          | Use case                          |
| ---------------- | ----- | --------------- | --------------------------------- |
| `authly`         | light | Indigo 600      | Clean, neutral                    |
| `rose`           | light | Rose 600        | Pinkish, warm                     |
| `mint`           | light | Teal 500        | Fresh, calm                       |
| `purple-night`   | dark  | Fuchsia 500     | Default. Modern dark with magenta pop |
| `midnight-blue`  | dark  | Cyan 500        | Modern dark with bluish tone      |
| `obsidian`       | dark  | Pure white      | High-contrast monochrome          |

Architecture (pure CSS, no UI framework):

- Activation. `data-theme="..."` on `<html>` set BEFORE first paint by an inline script in `index.html` that reads `localStorage["authly:theme"]` and falls back to default. Zero FOUC on reload.
...
- Components. Hand-written semantic CSS classes in `src/index.css` (`.btn`, `.btn--primary`, `.form-card`, `.welcome`, `.tile`, `.feature-card`, etc.) all reading from those tokens.
- Registry and state. `src/styles/themes/index.js` is the single source of truth. `ThemeProvider` and `useTheme()` persist the choice.
- Switcher. `ThemePicker.jsx` is a swatch-grid modal grouped Dark and Light, instant-apply, no Save button.
...
| --------------- | -------- | -------------------------------------------------------- | ------------------------------ |
| `NODE_ENV`      | no       | `development` (default) or `production`                  | `development`                  |
| `PORT`          | yes      | HTTP port for the API                                     | `5000`                         |
| `APP_NAME`      | no       | Brand shown in emails and UI                              | `Authly`                       |
...
| `JWT_SECRET`    | yes      | 64-byte hex used to sign access tokens                    | `npm run gen-secrets` output    |
| `REFRESH_SECRET`| yes      | 64-byte hex used to sign refresh tokens (must differ)     | `npm run gen-secrets` output    |
...
| `VITE_APP_NAME`  | no       | Brand shown in navbar and page titles        | `Authly`                 |

## Documentation

- [API reference](docs/API.md). Every endpoint with payloads and error codes.
...
| `/home`                       | protected             | Dashboard. Stats, account, session, activity, quick actions          |
| `/login`                      | public-only           | Email and password (then OTP)                                        |
...
| `/dashboard`                  | protected (admin)     | Total users and system status                                        |
| `*`                           | public                | 404                                                                  |

## Roadmap

- Per-device session list and remote sign-out
- WebAuthn or passkey support as a stronger 2FA factor
...
[MIT](LICENSE) (c) 2026 [Aryan Shakya](https://github.com/aryanshakya06)
